...
const webApi: ElectronAPI = {
  ...
  // ── AI Chat ──
  // Google Gemini uses API-key-in-URL which works from the browser.
  // Anthropic, OpenAI, Groq, OpenRouter block browser requests via CORS.
  // For those providers, return a clear error directing users to the desktop app.
  sendChat: async (params: {
    ...
    // Only Google Gemini supports browser CORS (API key in URL, no auth header restriction)
    if (provider !== 'google') {
      return { error: `AI Chat for ${provider} is only available in the desktop app (browser CORS restrictions prevent direct API calls). Google Gemini works in the browser — switch provider or use the desktop app.` };
    }
