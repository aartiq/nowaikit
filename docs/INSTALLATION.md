...
| Variable | Default | Description |
|----------|---------|-------------|
| `NOWAIKIT_CLAUDE_BIN` | `claude` | Path to the `claude` executable when using `--provider claude-cli` |
| `NOWAIKIT_CODEX_BIN` | `codex` | Path to the `codex` executable when using `--provider codex-cli` |
